...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == COPY_KEYS_BUTTON_SECTION && indexPath.row == 0) {
        if (!([networkDetails objectForKey:@"WPA_IE"] || [[networkDetails objectForKey:@"WEP"] boolValue])) {
            UIAlertView *msgBox = [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"vulnerable_ap_title", @"AP Posiblemente vulnerable") message:NSLocalizedString(@"no_key_ap_message", @"El AP no tiene clave.") delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil] autorelease];
            [msgBox show];
        } else {
            ...
            if (wlanKeys != nil && [wlanKeys count] == 1) {
                ...
                UIAlertView *msgBox = [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"safe_ap_title", @"AP seguro, clave no encontrada") message:NSLocalizedString(@"safe_ap_message", @"No se pudo encontrar la clave, el AP no tiene una clave que pueda ser calculada mediante sus datos publicos.") delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil] autorelease];
                [msgBox show];
            }
        }
